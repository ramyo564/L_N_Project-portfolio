# Case 5. 비동기 발행 데코레이터로 http_req_failed.rate 0.93% -> 0%, p95 488ms -> 124ms 개선
...
Case 5는 같은 `1000VU` 쓰기 부하에서 `convertAndSend` 발행 스레드가 `요청 스레드(tomcat-handler/http-nio)`에서 `전용 발행 스레드(rabbit-pub/rabbit-publisher)`로 분리되며, raw summary 기준 `http_req_failed.rate 0.93% -> 0%`, `p95 488ms -> 124ms`로 개선되는 것을 증명한다.
...
정량 수치는 `summary.json`/최종 k6 summary를 우선하고, `report.html` hover 캡처 값은 보조 증거로만 사용한다.
