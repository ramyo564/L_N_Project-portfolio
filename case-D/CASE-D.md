...
실패한 TODO와 업무 습관을 분석해 실행 계획으로 바꾸는 AI 흐름이 하나의 요청 경로에 묶여 있어, LLM 응답이 지연되면 분석과 핵심 로직 처리가 함께 영향받기 쉬운 구조였습니다. 또한 세션을 닫았다가 다시 돌아와도 최신 분석 상태를 이어 받을 수 있는 복구 경로가 필요했습니다.
